...
# LOGGING --
logger = Logger(module_name = __name__, class_name = 'Production').logger


# BLOCK
class Production(Block):
    """Blocco infrastrutturale produttivo (centrali elettriche, fabbriche, fattorie).
    ...
    Context.BLOCK_INFRASTRUCTURE_ASSET['Production'].
    """
    ...
    def __init__(
        ...
        Args:
            name: Base name (prefissato con 'Production.'; se assente 'Unnamed_Production_#nnnn')
            ...
            sub_category: sotto-categoria infrastrutturale, validata contro
                          Context.BLOCK_INFRASTRUCTURE_ASSET['Production'] se fornita
        ...
        Raises:
            ValueError: sub_category fornita ma non ammessa per Production (oltre alle
                        validazioni di Block.__init__).
        """
        super().__init__(
            name=f"Production.{name}" if name else setName('Unnamed_Production'),
        ...
        validate_infrastructure_sub_category('Production', sub_category)
