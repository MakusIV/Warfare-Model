...
from Code.Dynamic_War_Manager.Source.Utility import Utility, Sphere, Hemisphere, random
...
class Military(Block) :    
    ...
    def __init__(self, block: Block, mil_category: str, name: str|None, side: str|None, description: str|None, category: str|None, sub_category: str|None, functionality: str|None, value: int|None, acp: Payload|None, rcp: Payload|None, payload: Payload|None, region: Region|None):   
            ...
            if not name:
                self._name = Utility.setName('Unnamed_Military')

            else:
                self._name = "Military." + name
    ...
    def checkParam(mil_category: str) -> bool: # type: ignore
        ...
        if not isinstance(mil_category, str) and mil_category not in Military_CATEGORY:
            return (False, f"Bad argument: mil_category {0} must be a MIL_CATEGORY string: {1}".format(mil_category, str([cat for cat in Military_CATEGORY])))
    ...
    def getRecon(self) -> Dict:
        ...
        for enmy_block in enemy_blocks:
            ...
            if self.mil_category in Military_CATEGORY["Ground Base"]:
            ...
            elif self.mil_category in Military_CATEGORY["Air Base"]:
                ...
                raise Exception( "Bad argument: mil_category {0} must be a MIL_CATEGORY[/'Ground Base/'] or MIL_CATEGORY[/'Air Base/'] string: {1}".format(self.mil_category, str( [cat for cat in [Military_CATEGORY["Ground Base"], Military_CATEGORY["Air Base"]]]) ))
    ...
    def isAirbase(self):
        return self._mil_category in Military_CATEGORY["Air Base"]
    
    def isGroundBase(self):        
        return self._mil_category in Military_CATEGORY["Ground Base"]
    
    def isNavalGroup(self):        
        return self._mil_category in Military_CATEGORY["Naval Base"]    
