...
from Code.Context import GROUND_COMBAT_EFFICACY, AIR_DEFENCE_ASSET, GROUND_MIL_BASE_VEHICLE_ASSET, BLOCK_ASSET_CATEGORY, BLOCK_INFRASTRUCTURE_ASSET
...
class Vehicle(Mobile) :    
    ...
    def loadAssetDataFromContext(self) -> bool:
        ...
        if self.block.isMilitary():
            asset_data = GROUND_MIL_BASE_VEHICLE_ASSET
            asset_data_air_defence = AIR_DEFENCE_ASSET

            for k, v in asset_data[self.category]: # block_class = "Mil_Base", category = "Armor", asset_type = "Infantry_Fighting_Vehicle"                       
            ...
            for k, v in asset_data_air_defence[self.category]: # block_class = "Mil_Base", category = "SAM_Big", asset_type = "Track_Radar"            
        ...
        if asset_type and (isinstance(asset_type, str)):        
            ...
            if self.block.block_class == "Mil_Base": # asset is a Mil_Base component
                ...
                if category:
                    ...
                    struct_asset = BLOCK_ASSET_CATEGORY["Block_Infrastructure_Asset"]["Mil_Base"][category].keys()
                    ...
                    vehicle_asset = BLOCK_ASSET_CATEGORY["Ground_Mil_Base Vehicle Asset"].items().keys()
                    air_defence_asset = BLOCK_ASSET_CATEGORY["Air_Defence_Asset_Category"].items().keys()
                    struct_asset = BLOCK_ASSET_CATEGORY["Block_Infrastructure_Asset"]["Mil_Base"].items().keys()
