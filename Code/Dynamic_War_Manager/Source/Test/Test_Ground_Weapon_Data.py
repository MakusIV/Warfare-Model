...
class TestGetWeaponScoreTarget(unittest.TestCase):
    ...
    def test_mixed_valid_invalid_target_type_raises_value_error(self):
        ...
        with self.assertRaises(ValueError):
            get_weapon_score_target("2A46M", ["Armored", "UNKNOWN_XYZ"], ["big"])
    ...
    def test_mixed_valid_invalid_target_dimension_raises_value_error(self):
        ...
        with self.assertRaises(ValueError):
            get_weapon_score_target("2A46M", ["Armored"], ["big", "UNKNOWN_DIM_XYZ"])

    def test_generic_target_type_collapses_to_structure(self):
        """'Generic' (fallback per target sconosciuti a monte, es. Air_Resources_Assigner)
        non solleva più errore: viene collassato su 'Structure' da WEAPON_TARGET_CLASS_MAP
        e produce lo stesso punteggio di 'Structure' passato direttamente."""
        score_generic = get_weapon_score_target("2A46M", ["Generic"], ["big"])
        score_structure = get_weapon_score_target("2A46M", ["Structure"], ["big"])
