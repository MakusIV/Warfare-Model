...
from Dynamic_War_Manager.Source.Context.Campaign_History import CampaignHistory
...
def _make_report(**overrides) -> dict:
    """Minimal status_report dict, optionally overriding fields."""
    base = {
        "block_name": "Alpha",
        "block_id": "B001",
        "side": "Blue",
...
class TestCampaignHistory(unittest.TestCase):
    ...
    def test_add_block_snapshot_creates_mission(self):
        ...
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        self.assertIn(self.M1, self.ch)

    def test_add_block_snapshot_stores_report(self):
        """The stored report is identical to the one passed in."""
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        stored = self.ch.get_block_snapshot(self.M1, self.R1, self.B1)
    ...
    def test_add_block_snapshot_same_mission_two_regions(self):
        ...
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R2, self.B2, self.report2)
        self.assertEqual(
            self.ch.get_block_snapshot(self.M1, self.R1, self.B1), self.report1
        )
        self.assertEqual(
            self.ch.get_block_snapshot(self.M1, self.R2, self.B2), self.report2
        )
    ...
    def test_add_block_snapshot_overwrites_same_block(self):
        ...
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, updated)
        self.assertEqual(
            self.ch.get_block_snapshot(self.M1, self.R1, self.B1)["state"], "Destroyed"
        )
    ...
    def test_add_block_snapshot_invalid_mission_id_type(self):
        with self.assertRaises(TypeError):
            self.ch.add_block_snapshot(123, self.D1, self.T1, self.R1, self.B1, self.report1)

    def test_add_block_snapshot_empty_mission_id(self):
        with self.assertRaises(TypeError):
            self.ch.add_block_snapshot("", self.D1, self.T1, self.R1, self.B1, self.report1)

    def test_add_block_snapshot_invalid_date_format(self):
        with self.assertRaises(ValueError):
            self.ch.add_block_snapshot(self.M1, "10/06/2024", self.T1, self.R1, self.B1, self.report1)

    def test_add_block_snapshot_invalid_time_format(self):
        with self.assertRaises(ValueError):
            self.ch.add_block_snapshot(self.M1, self.D1, "6h30", self.R1, self.B1, self.report1)

    def test_add_block_snapshot_empty_region_name(self):
        with self.assertRaises(TypeError):
            self.ch.add_block_snapshot(self.M1, self.D1, self.T1, "", self.B1, self.report1)

    def test_add_block_snapshot_empty_block_id(self):
        with self.assertRaises(TypeError):
            self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, "", self.report1)

    def test_add_block_snapshot_invalid_report_type(self):
        with self.assertRaises(TypeError):
            self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, "not a dict")

    def test_add_block_snapshot_conflicting_date(self):
        """Adding the same mission_id with a different date raises ValueError."""
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        with self.assertRaises(ValueError):
            self.ch.add_block_snapshot(self.M1, self.D2, self.T1, self.R1, self.B2, self.report2)

    def test_add_block_snapshot_conflicting_time(self):
        """Adding the same mission_id with a different time raises ValueError."""
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        with self.assertRaises(ValueError):
            self.ch.add_block_snapshot(self.M1, self.D1, self.T2, self.R1, self.B2, self.report2)
    ...
    def test_add_region_snapshot_stores_all_blocks(self):
        ...
        self.ch.add_region_snapshot(self.M1, self.D1, self.T1, self.R1, blocks)
        self.assertEqual(self.ch.get_block_snapshot(self.M1, self.R1, self.B1), self.report1)
        self.assertEqual(self.ch.get_block_snapshot(self.M1, self.R1, self.B2), self.report2)

    def test_add_region_snapshot_invalid_blocks_type(self):
        with self.assertRaises(TypeError):
            self.ch.add_region_snapshot(self.M1, self.D1, self.T1, self.R1, [self.report1])
    ...
    def test_missions_empty(self):
        self.assertEqual(self.ch.missions, [])

    def test_missions_insertion_order(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        self.ch.add_block_snapshot(self.M2, self.D2, self.T2, self.R1, self.B1, self.report2)
        self.assertEqual(self.ch.missions, [self.M1, self.M2])
    ...
    def test_len_after_inserts(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        self.ch.add_block_snapshot(self.M2, self.D2, self.T2, self.R1, self.B1, self.report2)
        self.assertEqual(len(self.ch), 2)
    ...
    def test_contains_false(self):
        self.assertNotIn("NONEXISTENT", self.ch)
    ...
    def test_get_mission_returns_correct_metadata(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        mission = self.ch.get_mission(self.M1)
    ...
    def test_get_mission_missing_raises_key_error(self):
        with self.assertRaises(KeyError):
            self.ch.get_mission("NONEXISTENT")

    def test_get_mission_invalid_id_type(self):
        with self.assertRaises(TypeError):
            self.ch.get_mission(42)
    ...
    def test_get_region_snapshot_returns_blocks(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        snap = self.ch.get_region_snapshot(self.M1, self.R1)
    ...
    def test_get_region_snapshot_missing_region_returns_empty_dict(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        snap = self.ch.get_region_snapshot(self.M1, "NonExistentRegion")
    ...
    def test_get_block_snapshot_missing_block_returns_none(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        self.assertIsNone(self.ch.get_block_snapshot(self.M1, self.R1, "GHOST"))

    def test_get_block_snapshot_missing_region_returns_none(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        self.assertIsNone(self.ch.get_block_snapshot(self.M1, "GHOST", self.B1))
    ...
    def test_get_block_history_empty_when_no_missions(self):
        self.assertEqual(self.ch.get_block_history(self.R1, self.B1), [])

    def test_get_block_history_single_entry(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        history = self.ch.get_block_history(self.R1, self.B1)
    ...
    def test_get_block_history_sorted_by_date_then_time(self):
        ...
        # Insert out of order: M2 (later) before M1 (earlier)
        self.ch.add_block_snapshot(self.M2, self.D2, self.T2, self.R1, self.B1, self.report2)
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        history = self.ch.get_block_history(self.R1, self.B1)
    ...
    def test_get_block_history_skips_missing_block(self):
        ...
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        self.ch.add_block_snapshot(self.M2, self.D2, self.T2, self.R1, self.B2, self.report2)
        history = self.ch.get_block_history(self.R1, self.B1)
        self.assertEqual(len(history), 1)
        self.assertEqual(history[0][0], self.M1)
    ...
    def test_get_block_history_invalid_region_type(self):
        with self.assertRaises(TypeError):
            self.ch.get_block_history(123, self.B1)

    def test_get_block_history_invalid_block_id_type(self):
        with self.assertRaises(TypeError):
            self.ch.get_block_history(self.R1, 456)
    ...
    def test_load_restores_full_history(self):
        self.ch.add_block_snapshot(self.M1, self.D1, self.T1, self.R1, self.B1, self.report1)
        self.ch.add_block_snapshot(self.M2, self.D2, self.T2, self.R2, self.B2, self.report2)
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as f:
            path = f.name
        try:
            self.ch.save(path)
            loaded = CampaignHistory.load(path)
