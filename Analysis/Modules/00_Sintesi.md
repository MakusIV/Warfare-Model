...
3. **Route/Edge/Waypoint: tre implementazioni incompatibili** (`DataType`, `Air_Route_Manager.py` locale, `Ground_Route_Manager.py` locale) senza alcun ponte. **⏳ APERTO — chiarimento in corso.** Precisazioni emerse: `Region.add_route`/`get_route`/`get_shortest_route`/`get_safest_route` operano davvero su `DataType.Route`; `Tactical_Evaluation.evaluateGroundRouteDangerLevel` referenzia `DataType.Route`/`Edge.calcTravelTime()` — quindi `DataType` non è isolata come sembrava, ma è comunque non funzionante: `Edge.__init__` ha due bug distinti (`self.calcLenght(self)` — nome sbagliato, il metodo reale si chiama `calcLength` e non vuole argomenti extra — e `self.calcTravelTime(self)` — nome giusto ma argomento extra indebito), ed `evaluateGroundRouteDangerLevel` stessa itera erroneamente `Route.edges` (la classe, non il parametro `route.edges`). Nessuno chiama oggi `evaluateGroundRouteDangerLevel`. `Ground_Route_Manager.py` ha già proprie classi locali `Waypoint`/`Edge`/`NavigationGraph` (non solo Dijkstra come si credeva). Resta da decidere il modello canonico con questo quadro completo.
