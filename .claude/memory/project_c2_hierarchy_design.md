...
## Campaign-level session arbiter — CONFIRMED 2026-09-18
Name: **`Theater_Session_Manager`**, sitting in `Command/` alongside `Session.py`/`Session_Mission_Planner.py`, above both sides' `C2_Manager`. Confirmed responsibilities, kept deliberately "dumb" (assembly/scheduling only — never doctrine or targeting, which stay inside each side's own C2 hierarchy):
