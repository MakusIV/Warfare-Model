...
## Start here
- [Fase 2 (fog-of-war combat power) — PLAN READY, NOT IMPLEMENTED — re-present in full next session](project_fase2_recon_combat_power_plan.md) — 2026-09-15: Opus/high-effort Plan agent produced a verified 5-phase plan (new Combat_Power_Estimation.py module, users field, use_recon toggle); 7 open design questions block start; user's own follow-up observation (dimension is exclusively a weapon-target-effectiveness concept, not combat-power) likely resolves Q1 toward "no dimension in the calibration bucket" — confirm at session start
