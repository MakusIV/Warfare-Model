...
- [Air priority: target-specific loadout selection — COMPLETE, committed & pushed 2026-09-12](project_air_priority_target_specific_loadout.md) — items 3-7 + weighted-distribution half of item 8 all done; route_length/route_speed explicitly deferred indefinitely, do not re-raise; plus a `_target_affinity` diagnostic table (`STAMPA_TARGET_AFFINITY` flag in Test_Region.py, 4 scenarios) — user will ask to extend scenarios in a future session
