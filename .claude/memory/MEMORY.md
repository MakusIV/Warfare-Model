...
- [Air priority: target-specific loadout selection — DESIGN READY, not implemented](project_air_priority_target_specific_loadout.md) — 2026-09-11: full design for combat_score_target_effectiveness + loadout-availability cascade (year/doctrine/stock); found+fixed 3 live bugs along the way (Military.get_military_category always "Naval_Base", TARGET_CLASSIFICATION dict_keys, Air_Resources_Assigner dimension-vocab ValueError) + unified the 3 dimension vocabularies to big/med/small (commit 6ab1e6dc) — only the 26→12 target-class collapse map is still open for this design
