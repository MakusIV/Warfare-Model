...
metadata:
  ...
  modified: 2026-09-16T17:55:10.759Z
---
...
Fase 6 (il nucleo di scoring: `_select_weight`, `_calculate_priority`, `_calc_surface_priority`, `_calc_air_priority` insieme — sono una catena di chiamate mutue, vanno spostate insieme; introduzione di `recon_cp_snapshot` esplicito al posto di `use_recon`+attributo di istanza; **con test di caratterizzazione numerica** come richiesto dall'utente; **verificare ancora lo stesso pattern**: altri test/diagnostici che patchano `Region._calculate_priority`/`Region._select_weight` direttamente), Fase 7 (`_calc_attack_priority`/`_calc_defense_priority` + `update_military_priorities`, `route_provider`, eliminazione `_get_tuple_hashable_block_item`), Fase 8 (pulizia finale: rimuovere le 2 docstring `""" superata """` morte, riordino sezioni). Suite verde ad ogni commit.
